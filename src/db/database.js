...
if (TURSO_URL) {
  ...
  const wrapper = {
    ...
    async batch(stmts, mode) {
      const transforms = stmts.map(s => {
        if (typeof s === 'string') return { kind: 'execute', sql: s };
        return { kind: 'execute', sql: s.sql || s, args: s.args || [] };
      });
      await this._client.batch(transforms, mode || 'write');
    },
